...
(async () => {
    ...
    for (let pageNum = 1; pageNum <= pdf.numPages; pageNum++) {
        ...
        const itemsXml = textContent.items.map(item => {
            ...
            return `<text value="${escapeXml(item.str)}" x="${finalX.toFixed(2)}" y="${finalY.toFixed(2)}" width="${width.toFixed(2)}" height="${height.toFixed(
                2,
            )}" font="${item.fontName}" />`;
        });

        pagesXml.push(`<page number="${pageNum}" width="${viewport.width}" height="${viewport.height}">\n  ${itemsXml.join("\n  ")}\n</page>`);
    }
